...
class MyCap:
	def __init__(self,filename):
		...
		self.frameWidth = int(self.get(cv2.CAP_PROP_FRAME_WIDTH))
		self.frameHeight = int(self.get(cv2.CAP_PROP_FRAME_HEIGHT))
...
print('Trying to open Videofile ' + args.filename)

#assert (exists(args.filename)), 'File not found'

	
#init video
cap = MyCap(args.filename)

if args.resolution:
	res = args.resolution.split('x')
	cap.setResolution(int(res[0]),int(res[1]))

print ("[Output Resolution] " + str(cap.frameWidth) + "x" + str(cap.frameHeight))
...
while(cap.isOpened()):
	...
	if ret == True:
		if args.resolution:
			frame = cv2.resize(frame, (cap.frameWidth,cap.frameHeight))
		if queuepulls ==1:
			timer2 = time.time()
		# Capture frame-by-frame
		#frame = frame.array
